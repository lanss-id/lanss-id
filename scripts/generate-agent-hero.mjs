...
// Source photo is a background-removed PNG (transparent outside the
// subject). We flatten it onto white first so the empty space renders as
// blank instead of noise, then crop tight around the subject. Adjust these
// four crop numbers if a future portrait has different framing/dimensions.
const portraitCrop = "crop=440:500:30:0";
const portraitFilter = [
  portraitCrop,
  "format=gray",
  "eq=contrast=1.15:brightness=0.03:gamma=0.96",
  "unsharp=3:3:0.35"
].join(",");

function buildProfileLines(projects) {
  const shortNames = {
    "Government Budget Dashboard": "Gov Dashboard",
    "Bank Transaction Clustering": "Bank Clustering",
    "Portfolio (mkp.dev)": "Portfolio"
  };

  return [
    { type: "header", value: "alan@build" },
    { type: "row", key: "Name", value: "Maulana Kayyis Purnadiva" },
    { type: "row", key: "Role", value: "Fullstack Engineer" },
    { type: "row", key: "Based", value: "Bandung, Indonesia" },
    { type: "row", key: "Mode", value: "Building / Shipping / Automating" },
    { type: "blank" },
    { type: "section", value: "BUILD.FOCUS" },
    { type: "row", key: "Product", value: "Idea to production system" },
    { type: "row", key: "AI", value: "Claude API, RAG, agents" },
    { type: "row", key: "Infra", value: "Docker, Oracle Cloud" },
    { type: "row", key: "Data", value: "PostgreSQL, pgvector" },
    { type: "blank" },
    ...
    { type: "blank" },
    { type: "footer", value: "SHIPPED SOLO, RUNNING IN PRODUCTION" }
  ];
}

// Monochrome only: every color reference below is a shade of black, white,
// or gray. No hue is used anywhere in either palette.
const palettes = {
  dark: {
    ...
    green: "#B5B5B5",
    red: "#FFFFFF",
    portraitStart: "#FFFFFF",
    portraitEnd: "#9E9E9E",
    scanBlend: "screen"
  },
  light: {
    backgroundStart: "#F7F7F5",
    backgroundEnd: "#EAEAE6",
    panel: "#FFFFFF",
    primary: "#161616",
    muted: "#6E6E6E",
    cyan: "#1A1A1A",
...
const layouts = {
  desktop: {
    ...
    infoTitle: { x: 524, y: 100 },
    portrait: { columns: 96, rows: 64, x: 78, y: 104, lineHeight: 6.65, fontSize: 6.5 },
    portraitClip: { x: 24, y: 82, width: 470, height: 438, radius: 12 },
    system: { x: 528, y: 125, width: 620, lineHeight: 21.5, fontSize: 14 },
    footerY: 585
  },
  mobile: {
    width: 720,
    ...
    infoTitle: { x: 66, y: 530 },
    portrait: { columns: 84, rows: 54, x: 180, y: 148, lineHeight: 5.7, fontSize: 6.6 },
    portraitClip: { x: 58, y: 122, width: 604, height: 312, radius: 12 },
    system: { x: 72, y: 565, width: 574, lineHeight: 21, fontSize: 13 },
    footerY: 1045
  }
};
...
function getSourcePath() {
  ...
  if (sourceIndex === -1 || !process.argv[sourceIndex + 1]) {
    throw new Error("Usage: node scripts/generate-agent-hero.mjs --source /absolute/path/to/portrait.png");
  }
...
function buildSystemLayer({ x, y, lineHeight, fontSize }, colors, profileLines) {
  ...
  profileLines.forEach((line, index) => {
    ...
    const dots = ".".repeat(Math.max(3, 16 - line.key.length));
    rows.push(
...
<title id="title">Maulana Kayyis Purnadiva - Fullstack Engineer</title>
<desc id="description">A builder profile card with Alan's high-contrast monochrome ASCII portrait, build focus, and selected work.</desc>
<defs>
...
<text x="${titleCenter}" y="${titlebar.y + titlebar.height / 2 + 5}" text-anchor="middle" class="terminal-label">alan@build ~ % ./profile</text>
...
<text x="${layout.visualTitle.x}" y="${layout.visualTitle.y}" class="panel-title">PORTRAIT / ALAN</text>
...
${system}
<text x="${layout.width / 2}" y="${layout.footerY}" text-anchor="middle" class="mono" font-size="10" letter-spacing="1.5" fill="${colors.muted}">FULLSTACK ENGINEERING / AI INTEGRATION / AUTOMATION</text>
...
const outputs = [
  { filename: "builder-profile-dark.svg", mode: "dark", size: "desktop" },
  { filename: "builder-profile-light.svg", mode: "light", size: "desktop" },
  { filename: "builder-profile-mobile-dark.svg", mode: "dark", size: "mobile" },
  { filename: "builder-profile-mobile-light.svg", mode: "light", size: "mobile" }
];
